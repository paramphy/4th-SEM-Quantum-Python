...
psi_p = np.zeros(
    N,
)  # Observable probability (magnitude-squared
...
for t in range(T + 1):
    ...
    psi_i[FU, IDX1] = psi_i[PA, IDX1] + c1 * (
        psi_rPR[IDX2] - 2 * psi_rPR[IDX1] + psi_rPR[IDX3]
    )
    psi_i[FU] -= c2V * psi_r[PR]

    psi_r[FU, IDX1] = psi_r[PA, IDX1] - c1 * (
        psi_iPR[IDX2] - 2 * psi_iPR[IDX1] + psi_iPR[IDX3]
    )
